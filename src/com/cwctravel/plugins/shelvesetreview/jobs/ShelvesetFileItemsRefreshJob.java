...
public class ShelvesetFileItemsRefreshJob extends Job {
	...
	protected IStatus run(IProgressMonitor monitor) {
		...
		PendingSet[] pendingSets = vC.queryShelvedChanges(this.shelvesetItem.getName(), this.shelvesetItem.getOwnerName(), null, true, null);
				...
				if(pendingChanges != null) {
					for(PendingChange pendingChange: pendingChanges) {
						...
						if(itemType == ItemType.FILE) {
							ShelvesetFileItem shelvesetResourceItem = new ShelvesetFileItem(this.shelvesetItem, pendingSet, pendingChange);
								...
								if(viewReferences != null) {
									for(IViewReference viewReference: viewReferences) {
										...
										if(viewPart instanceof ShelvesetNavigator) {
											...
											if(expand) {
												shelvesetNavigator.getCommonViewer().expandToLevel(ShelvesetFileItemsRefreshJob.this.shelvesetItem, 1);
											}
											else {
												shelvesetNavigator.getCommonViewer().refresh(ShelvesetFileItemsRefreshJob.this.shelvesetItem, true);
											}
										}
									}
								}
